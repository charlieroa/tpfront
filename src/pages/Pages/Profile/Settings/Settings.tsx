...
const ServiceModal: React.FC<{
  ...
  tenantId: string;
  edit?: Service | null;
}> = ({ isOpen, onClose, onSaved, categories, onCategoryCreated, onCategoryDeleted, tenantId, edit }) => {
  ...
  const createCategoryInline = async () => {
    ...
    try {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: e?.response?.data?.message || e?.message || 'No se pudo crear la categoría',
      });
    } finally { setSaving(false); }
  };

  const deleteCategory = async (catToDelete: Category, e: React.MouseEvent) => {
    e.stopPropagation();
    
    const result = await Swal.fire({
        title: `¿Eliminar "${catToDelete.name}"?`,
        text: "Los servicios en esta categoría no se eliminarán, pero quedarán sin categoría.",
        icon: 'warning',
  ...
  const save = async () => {
    if (!categoryId || !name.trim() || !price || !duration) {
      Swal.fire({
          ...
          text: 'Por favor completa categoría, nombre, precio y duración.',
      });
      return;
    }
    const body: any = {
    ...
    try {
      ...
      // --- MODIFICADO: Añadida la alerta de éxito ---
      const successTitle = edit ? '¡Servicio actualizado!' : '¡Servicio Creado!';
      Swal.fire({
        icon: 'success',
        title: successTitle,
      ...
      onClose();
        ...
        Swal.fire({
            ...
            text: e?.response?.data?.message || e?.message || 'No se pudo guardar el servicio'
        });
    } finally { setSaving(false); }
  };
...
const Settings: React.FC = () => {
        ...
        try {
          ...
          Swal.fire({
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error al Guardar',
                text: e?.response?.data?.message || e?.message || "No se pudieron guardar los cambios."
            });
        } finally {
          setSaving(false);
        }
        ...
        if (result.isConfirmed) {
            try { 
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: e?.response?.data?.message || e?.message || 'No se pudo eliminar el servicio'
                });
            }
        }
    };
    ...
    return (
      <React.Fragment>
        {/* ... (Todo el JSX de la página de Settings sin cambios) ... */}
        <div className="page-content">
          <Container fluid>
                ...
                <Card className="mt-xxl-n5">
                  ...
                  <CardBody className="p-4">
                    ...
                    <TabContent activeTab={activeTab}>
                      <TabPane tabId="1">
                        <DatosTenant section="datos" name={name} phone={phone} address={address} email={email} website={website} ivaRate={ivaRate} adminFee={adminFee} setName={setName} setPhone={setPhone} setAddress={setAddress} setEmail={setEmail} setWebsite={setWebsite} setIvaRate={setIvaRate} setAdminFee={setAdminFee} perDay={perDay} toggleDay={() => {}} changeHour={() => {}} applyMondayToAll={() => {}} saving={saving} onSubmit={handleSaveInfo} onCancel={() => updateStateFromTenant(tenant)} />
                      </TabPane>
                      <TabPane tabId="2">
                        <DatosTenant section="horario" name={name} phone={phone} address={address} email={email} website={website} ivaRate={ivaRate} adminFee={adminFee} setName={() => {}} setPhone={() => {}} setAddress={() => {}} setEmail={() => {}} setWebsite={() => {}} setIvaRate={() => {}} setAdminFee={() => {}} perDay={perDay} toggleDay={toggleDay} changeHour={changeHour} applyMondayToAll={applyMondayToAll} saving={saving} onSubmit={handleSaveHours} onCancel={() => updateStateFromTenant(tenant)} />
                      </TabPane>
                      <TabPane tabId="3">
                        ...
                        <ServiceModal isOpen={svModalOpen} onClose={() => setSvModalOpen(false)} onSaved={refreshAllServices} categories={categories} onCategoryCreated={(c) => { setCategories((prev) => [...prev, c].sort((a,b) => a.name.localeCompare(b.name))); }} onCategoryDeleted={handleCategoryDeleted} tenantId={tenantId} edit={svEdit} />
                      </TabPane>
                      <TabPane tabId="4">
                        <Personal />
                      </TabPane>
