    ...
    return (
      <div>
        <div className="d-flex align-items-center flex-wrap gap-2 mb-2">
          <Badge pill color={catFilter === "all" ? "primary" : "light"} className={catFilter === "all" ? "" : "text-dark"} style={{ cursor: "pointer" }} onClick={() => onCatFilter("all")}>
            Todas las categorías
          </Badge>
          {categories.map(c => (
            <Badge key={c.id} pill color={catFilter === c.id ? "primary" : "light"} className={catFilter === c.id ? "" : "text-dark"} style={{ cursor: "pointer" }} onClick={() => onCatFilter(c.id)}>
              {c.name}
            </Badge>
          ))}
        </div>
        ...
        <UncontrolledDropdown isOpen={open} toggle={() => setOpen(v => !v)}>
          ...
          <DropdownMenu className="p-2" style={{ width: "100%", maxHeight: 360, overflowY: "auto" }}>
            <div className="mb-2">
              <Input innerRef={searchRef} placeholder="Buscar servicios…" value={query} onChange={(e) => setQuery(e.target.value)} />
            </div>
            <div>
              {filtered.length === 0 && <div className="text-muted px-2 py-1">Sin resultados</div>}
              {filtered.map(s => {
                const checked = selectedIds.includes(s.id);
                const catName = categories.find(c => c.id === s.category_id)?.name || "—";
                return (
                  <button key={s.id} type="button" className="dropdown-item d-flex align-items-center justify-content-between" onClick={() => onToggle(s.id)}>
                    <div className="me-2 text-start">
                      <div className="fw-semibold">{s.name}</div>
                      <div className="small text-muted">{s.duration_minutes} min · ${s.price.toLocaleString()} · {catName}</div>
                    </div>
        ...
        <div className="mt-2">
          ...
          {selectedIds.map(id => {
            ...
            if (!s) return null;
            return (
              <Badge key={id} pill color="light" className="text-dark me-1 mb-1" style={{ cursor: "pointer" }} title="Quitar" onClick={() => onToggle(id)}>
                ...
                <Row className="g-3">
                    {!edit && (
                        <Col md={12}>
                            ...
                            <Input type="select" value={roleId} onChange={(e) => setRoleId(Number(e.target.value))}>
    ...
    return (
      <div>
        ...
        <div className="d-flex justify-content-between align-items-center mb-3">
            <div>
              <h5 className="mb-1">Personal</h5>
              <div className="d-flex gap-2 align-items-center">
                <small className="text-muted">
                  {roleFilter === "all" && `${staff.length} total`}
                  {roleFilter === 3 && `${stylistsCount} estilistas`}
                  {roleFilter === 2 && `${cashiersCount} cajeros`}
                </small>
                {roleFilter !== "all" && (
                  <Badge color="primary" pill style={{ cursor: "pointer" }} onClick={handleRoleFilterToggle} title="Click para cambiar filtro">
                    Filtrado
                  </Badge>
                )}
              </div>
        ...
        <div className="table-responsive">
            ...
            <thead>
                <tr>
                  <th>Nombre</th>
                  <th style={{ cursor: "pointer", userSelect: "none" }} onClick={handleRoleFilterToggle} title="Click para filtrar por rol">
                    Rol {roleFilter !== "all" && <i className="ri-filter-3-line ms-1"></i>}
                ...
                {paginatedStaff.length === 0 && (
                  <tr>
                    <td colSpan={7} className="text-center text-muted py-4">
                      {roleFilter === "all" ? "Sin personal registrado" : roleFilter === 3 ? "No hay estilistas registrados" : "No hay cajeros registrados"}
                    </td>
                  </tr>
                )}
