...
// Thunks locales (el modal despacha por su cuenta)
import {
...
interface AppointmentModalProps {
  isOpen: boolean;
  onClose: () => void;
  selectedEvent: any | null; // si existe, es edición
  defaultDate?: Date | null; // para creación desde clic en calendario
}
...
const AppointmentModal: React.FC<AppointmentModalProps> = ({ isOpen, onClose, selectedEvent, defaultDate }) => {
  ...
  // ————————————————————————————————————————
  // Reset y preparación al abrir/cerrar
  // ————————————————————————————————————————
  useEffect(() => {
    if (isOpen) {
      // Si es creación desde clic de fecha, precargar ese día
      if (!selectedEvent && defaultDate) {
        validation.setFieldValue("date", defaultDate);
      }
      // Si es edición, poblar
      if (selectedEvent) populateFormForEdit();
    } else {
  ...
  const validation = useFormik<AppointmentFormValues>({
    ...
    validationSchema: Yup.object({
      ...
      newClientEmail: Yup.string().when([], {
        is: () => showNewClientForm,
        then: (schema: any) => schema.email("Email inválido").required("El email es requerido."),
        otherwise: (schema: any) => schema.notRequired(),
      }),
    }),
    initialValues: {
  ...
  const removeExtraRow = (idx: number) => {
    ...
    setIsLoadingStylistsRow((prev) => {
      const c = { ...prev } as Record<number, boolean>;
      delete c[idx];
      return c;
    });
  ...
  const changeExtraRow = (idx: number, field: keyof ExtraRow, value: string) => {
    setExtraRows((prev: ExtraRow[]) => prev.map((r: ExtraRow, i: number) => (i === idx ? { ...r, [field]: value } : r)));
  };
  ...
  const handleSuggestForRow = async (idx: number) => {
    ...
    try {
      ...
      if (
        String(validation.values.stylist_id) === String(stylist.id) ||
        isStylistUsedElsewhere(stylist.id, idx)
      ) {
      ...
      toast.success(`Digiturno (fila ${idx + 2}) asignó a: ${stylist.first_name || "Estilista"} ${
        stylist.last_name || ""
      }`);
    } catch {
      // manejado por thunk
            ...
            <Col xs={12} className="mb-3">
              ...
              <div className="d-flex gap-2">
                <Input
                  ...
                  disabled={!validation.values.service_id || isLoadingStylists}
                >
                  <option value="">
